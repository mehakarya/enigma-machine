...
// encryption function
void encrypt(char *msg, char *startPosition) {
    ...
    for (int i = 0; i < strlen(msg); i++) {
        if (toupper(msg[i]) >= 65 && toupper(msg[i] <= 90)) {
            ...
            // encryption
            int temp = rotorRtoL(ROTOR_III, (((getPosition(toupper(msg[i])) + offset3) % 26) + 26) % 26);
            temp = rotorRtoL(ROTOR_II, (((temp + offset2 - offset3) % 26) + 26) % 26);
            temp = rotorRtoL(ROTOR_I, (((temp + offset1 - offset2) % 26) + 26) % 26);
            temp = (((reflect(REFLECTOR_B, (temp - offset1) % 26) + offset1) % 26) + 26) % 26;
            temp = (((rotorLtoR(ROTOR_I, temp) - offset1 + offset2) % 26) + 26) % 26;
            temp = (((rotorLtoR(ROTOR_II, temp) - offset2 + offset3) % 26) + 26) % 26;
            temp = (((rotorLtoR(ROTOR_III, temp) - offset3) % 26) + 26) % 26;
...
int main() {
    ...
    printf("Nice! What's the secret message (no spaces)?\n");
    scanf("%s", message);
